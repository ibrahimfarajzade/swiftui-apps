...
struct EmojiArtDocumentView: View {
    ...
    var body: some View {
        VStack {
            ...
            GeometryReader { geometry in
                        ...
                        ForEach(self.document.emojis) { emoji in
                            Text(emoji.text)
                                ...
                                .background(
                                    Circle()
                                        .stroke(Color.black)
                                        .opacity(self.isSelected(emoji) ? 1 : 0)
                                )
                                .gesture(self.dragEmojisGesture(for: emoji))
                                .gesture(self.longPressToRemove(emoji))
                ...
                .gesture(self.tapBgToDeselectOrZoom(in: geometry.size))
                .clipped()
                ...
                .onDrop(of: ["public.image", "public.text"], isTargeted: nil) { providers, location in
                    ...
                    location = CGPoint(x: location.x - self.panOffset.width, y: location.y - self.panOffset.width)
    ...
    private func dragEmojisGesture(for emoji: EmojiArt.Emoji) -> some Gesture {
        let isEmojiPartSelection = isSelected(emoji)
        
        return DragGesture()
            .updating($gestureDragEmojisOffset) { latestDragGestureValue, gestureDragEmojisOffset, transtion in
                gestureDragEmojisOffset = latestDragGestureValue.translation / self.zoomScale
            }
            .onEnded { finalDragGestureValue in
                let distanceDragged = finalDragGestureValue.translation / self.zoomScale
                
                if isEmojiPartSelection {
